# Vendored from huggingface/transformers v4.51.3:
#   src/transformers/pipelines/text2text_generation.py
# transformers v5 deleted these pipelines (see MIGRATION_GUIDE_V5.md); upstream suggests using a
# chat model instead, which is not an option for a serving layer that must run the seq2seq /
# QA models that already exist on the Hub. Everything they depend on still ships in v5.
# Changes from upstream, kept to the minimum so this stays diffable:
#   - relative imports rewritten as absolute `transformers.*` imports
#   - TensorFlow branches dropped (`self.framework` no longer exists in v5; image is torch-only)
#   - `modelcard` / `framework` constructor args dropped (removed from v5 `Pipeline.__init__`)
#   - `TruncationStrategy` imported from `transformers.tokenization_utils_base` (moved in v5)
#   - `_pipeline_calls_generate = True` declared (new v5 opt-in for generation pipelines)
# ruff: noqa
...
class Text2TextGenerationPipeline(Pipeline):
    ...
    # v5 only sets up self.generation_config / self.prefix / self.assistant_model for pipelines
    # that declare this (base.Pipeline defaults it to False); v4 did it unconditionally.
